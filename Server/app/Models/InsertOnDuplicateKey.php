...
trait InsertOnDuplicateKey
{
    ...
    public static function insertOnDuplicateKey(array $data, array $updateColumns = null)
    {
        ...
        foreach(array_chunk($data, 1000) as $sub_data) {
            $sql = static::buildInsertOnDuplicateSql($sub_data, $updateColumns);
            $sub_data = static::inLineArray($sub_data);
            self::getModelConnectionName()->affectingStatement($sql, $sub_data);
        }
        return true;
    }
    /**
    ...
    public static function insertIgnore(array $data)
    {
        ...
        foreach(array_chunk($data, 1000) as $sub_data) {
            $sql = static::buildInsertIgnoreSql($sub_data);
            $sub_data = static::inLineArray($sub_data);
            self::getModelConnectionName()->affectingStatement($sql, $sub_data);
        }
        return true;
